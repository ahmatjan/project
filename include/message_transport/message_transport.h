...
public:
    explicit MessageTransport(const ros::NodeHandle& nh, const std::string & package_name,
    ...
    Publisher advertise(const std::string& base_topic, uint32_t queue_size, bool latch = false) {
        return advertise(base_topic, queue_size, typename SingleSubscriberPublisher<M>::StatusCB(),
                typename SingleSubscriberPublisher<M>::StatusCB(), ros::VoidPtr(), latch);
    }
    ...
    Publisher advertise(const std::string& base_topic, uint32_t queue_size,
            ...
            const typename SingleSubscriberPublisher<M>::StatusCB& disconnect_cb =
                    SingleSubscriberPublisher<M>::StatusCB(), const ros::VoidPtr& tracked_object =
                    ros::VoidPtr(), bool latch = false) {
        Publisher p(nh_);
        p.do_initialise<M>(nh_, package_name_, class_name_, base_topic, queue_size, connect_cb,
    ...
    Subscriber subscribe(const std::string& base_topic, uint32_t queue_size,
        ...
        Subscriber s(nh_);
        s.do_subscribe<M>(nh_, package_name_, class_name_, base_topic, queue_size, callback,
     ...
     */
    Subscriber subscribe(const std::string& base_topic, uint32_t queue_size,
            void (*fp)(const typename M::ConstPtr&), const TransportHints& transport_hints =
                    TransportHints()) {
        return subscribe(base_topic, queue_size,
                boost::function<void(const typename M::ConstPtr&)>(fp), ros::VoidPtr(),
                transport_hints);
    }
    ...
    Subscriber subscribe(const std::string& base_topic, uint32_t queue_size,
            ...
            const TransportHints& transport_hints = TransportHints()) {
        return subscribe(base_topic, queue_size, boost::bind(fp, obj, _1), ros::VoidPtr(),
                transport_hints);
    }
    ...
    template<class T>
    Subscriber subscribe(const std::string& base_topic, uint32_t queue_size,
            void (T::*fp)(const typename M::ConstPtr&), const boost::shared_ptr<T>& obj,
            const TransportHints& transport_hints = TransportHints()) {
