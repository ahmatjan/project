    ...
    virtual void publish(const Base& message) const {
        if (!simple_impl_ || !simple_impl_->pub_) {
            ROS_ASSERT_MSG(false,
                    "Call to publish() on an invalid message_transport::SimplePublisherPlugin");
            return;
        }
...
protected:
    virtual void advertiseImpl(ros::NodeHandle & nh, const std::string& base_topic,
        ...
        simple_impl_->queue_size_ = queue_size;
        simple_impl_->pub_ = nh.advertise < M
                > (getTopicToAdvertise(base_topic), queue_size, bindCB(user_connect_cb,
                        &SimplePublisherPlugin::connectCallbackHandle), bindCB(user_disconnect_cb,
                        &SimplePublisherPlugin::disconnectCallbackHandle), tracked_object, latch
                        | forcelatch_);
    ...
    void publishInternal(const M& message) const {
        if (simple_impl_)
            simple_impl_->pub_.publish(message);
    }
...
private:
    ...
    void subscriberCB(const ros::SingleSubscriberPublisher& ros_ssp,
        ...
        MessagePublishFn message_publish_fn = boost::bind(pub_mem_fn, this, _1,
                bindInternalPublisher(ros_ssp));

        SingleSubscriberPublisher<Base> ssp(ros_ssp.getSubscriberName(), getTopic(),
                boost::bind(&SimplePublisherPlugin::getNumSubscribers, this), message_publish_fn);
        user_cb(ssp);
    }
