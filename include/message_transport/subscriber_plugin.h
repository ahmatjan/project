...
public:
    ...
    void subscribe(ros::NodeHandle& nh, const std::string& base_topic, uint32_t queue_size,
            ...
            const TransportHints& transport_hints = TransportHints()) {
        return subscribe(nh, base_topic, queue_size, boost::bind(fp, obj, _1), ros::VoidPtr(),
                transport_hints);
    }
    ...
    void subscribe(ros::NodeHandle& nh, const std::string& base_topic, uint32_t queue_size,
            ...
            const TransportHints& transport_hints = TransportHints()) {
        return subscribe(nh, base_topic, queue_size, boost::bind(fp, obj.get(), _1), obj,
                transport_hints);
    }
...
protected:
    ...
    virtual void subscribeImpl(ros::NodeHandle& nh, const std::string& base_topic,
            uint32_t queue_size, const Callback& callback, const ros::VoidPtr& tracked_object,
