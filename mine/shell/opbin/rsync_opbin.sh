...
do
    ...
    do
        remote_dir="/home/${user[$outter_idx]}/opbin/"
        local_dir="/home/yangkai04/shell/machine_opbin/${host[$outter_idx]}/"
        
        filenames=(`ssh ${user[$outter_idx]}@${host[$outter_idx]} " [ -d ${remote_dir} ] && cd ${remote_dir} && find . -name \"*${subfix}\""`)
        if [ -z "${filenames}" -a "${user[$outter_idx]}" != "work" ]
        then
            remote_dir="/home/work/opbin/"
            filenames=(`ssh ${user[$outter_idx]}@${host[$outter_idx]} " [ -d ${remote_dir} ] && cd ${remote_dir} && find . -name \"*${subfix}\""`)
        fi
